# Etch-a-Sketch

A browser-based sketching pad built as part of **The Odin Project** foundations curriculum. It features a dynamic grid, mouse-drag drawing, and a progressive darkening effect.

**[View Live Demo]()**

## How to Use
1.  **Draw:** Click and drag your mouse over the grid to leave a trail.
2.  **Darken:** Hovering over the same square multiple times darkens it by 10% each pass.
3.  **Reset:** Click the button at the top to clear the board and choose a new grid density (up to 100x100).

## Tech Stack
* HTML5
* CSS3 (Flexbox)
* JavaScript (DOM Manipulation)
